...
export async function archivePacersAssets({ root, dist }) {
  const manifestPath = resolve(root, 'src', 'data', 'pacers-archive.json');
  const manifest = JSON.parse(await readFile(manifestPath, 'utf8'));
  if (!Array.isArray(manifest.assets) || manifest.assets.length === 0) throw new Error('Pacers archive manifest contains no assets.');
  ...
  for (const asset of approved) {
    ...
    const response = await fetch(asset.remoteSrc, { redirect:'follow', headers:{'user-agent':'Era-Theory-Archive/1.0 (+https://era-theory.pages.dev/)'} });
  ...
  await writeFile(resolve(archiveDir, 'pacers-manifest.json'), JSON.stringify({version:manifest.version,policy:manifest.policy,assets:archived}, null, 2) + '\n');
  console.log(`Archived ${archived.length} rights-approved Indiana Pacers images.`);
  return archived;
}
