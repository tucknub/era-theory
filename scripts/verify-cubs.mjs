...
if(totals.seasons!==83||totals.evidence!==1039||totals.tests!==1200000)throw new Error(`Unexpected four-report totals: ${JSON.stringify(totals)}`);
if(cubs.finalScore!==80.7||cubs.coreEvidenceRecords!==135)throw new Error(`Unexpected Cubs registry audit: score ${cubs.finalScore}, evidence ${cubs.coreEvidenceRecords}.`);
for(const marker of ["A great championship window. Not a dynasty.","80.7","The failure wasn't the peak","135 sourced records","0 / 100,000","Did the teardown build the next contender?","91.1","73.9","96.0","62.6","81.1","Championship lifecycle — not a dynasty","elite foundation building and peak quality"])if(!report.includes(marker))throw new Error(`Cubs report missing marker: ${marker}`);
for(const marker of ['135 / 135','Eight MLB lifecycle questions','0 / 100,000','300,000 random-weight tests','Recent drafts are immature','135 of 135 core records carry a source'])if(!method.includes(marker))throw new Error(`Cubs methodology missing marker: ${marker}`);
for(const stale of ['160 / 160','160 sourced records','The Cubs lifecycle scores 80.6','<strong>55.1</strong><h3>draft / development score</h3>','Championship-caliber rebuild / great window','The balanced lifecycle model rewards the title and the 2015–18 run while penalizing replenishment and durability.'])if(report.includes(stale)||method.includes(stale))throw new Error(`Stale Report 004 research marker survived build: ${stale}`);
...
console.log(`Verified audited Report 004: ${published.length} published stories, ${totals.seasons} completed seasons, ${totals.evidence}/${totals.evidence} sourced core records, ${totals.tests.toLocaleString()} model tests and ${manifest.assets.length} rights-ledgered Cubs photographs.`);
