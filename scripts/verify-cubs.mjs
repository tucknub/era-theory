...
for(const marker of ["A great championship window. Not a dynasty.","80.7","The failure wasn't the peak","135 sourced records","0 / 100,000","Did the teardown build the next contender?","91.1","73.9","96.0","62.6","81.1"])if(!report.includes(marker))throw new Error(`Cubs report missing marker: ${marker}`);
...
for(const stale of ['160 / 160','160 sourced records','The Cubs lifecycle scores 80.6','<strong>55.1</strong><h3>draft / development score</h3>'])if(report.includes(stale)||method.includes(stale))throw new Error(`Stale Report 004 research marker survived build: ${stale}`);
